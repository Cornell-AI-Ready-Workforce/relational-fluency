...
def test_a_piped_rater_token_opens_the_console(client, rater_roster):
    r = client.get("/rate/start", params={"token": VALID_TOKEN},
                   follow_redirects=False)
    assert r.status_code == 307
    assert r.headers["location"] == f"/rate?token={VALID_TOKEN}"


@pytest.mark.parametrize("spelling", ["token", "rt", "rater_token"])
def test_the_rater_link_accepts_the_spellings_a_survey_might_pipe(client,
                                                                  rater_roster,
                                                                  spelling):
    r = client.get("/rate/start", params={spelling: VALID_TOKEN},
                   follow_redirects=False)
    assert r.status_code == 307


def test_an_unpiped_rater_field_is_named_as_a_piping_fault(client, rater_roster,
                                                           capsys):
    """The rater equivalent of the participant-key failure, and it is caught by
    the same validation: a literal ${e://Field/RaterToken} is not a token."""
    r = client.get("/rate/start", params={"token": "${e://Field/RaterToken}"},
                   follow_redirects=False)
    assert r.status_code == 200
    assert "did not carry a usable rater token" in r.text
    assert "unreplaced Qualtrics field" in capsys.readouterr().out


def test_a_link_with_no_token_asks_for_one_instead_of_refusing(client, rater_roster):
    """A rater who was told to click this link and gets a bare 401 has nowhere
    to go. The token they were emailed is the thing that fixes it, so ask."""
    r = client.get("/rate/start", follow_redirects=False)
    assert r.status_code == 200
    assert 'name="token"' in r.text
    assert "invitation" in r.text


def test_an_expired_token_says_so_and_still_offers_the_form(client, rater_roster):
    r = client.get("/rate/start", params={"token": UNKNOWN_TOKEN},
                   follow_redirects=False)
    assert r.status_code == 401
    assert "not valid any more" in r.text
    assert 'name="token"' in r.text


def test_the_rater_token_is_never_echoed_back_into_the_page(client, rater_roster):
    """It is a live credential. A rejected one still must not come back in the
    HTML, where it would land in a screenshot, a bug report or a proxy log."""
    r = client.get("/rate/start", params={"token": UNKNOWN_TOKEN},
                   follow_redirects=False)
    assert UNKNOWN_TOKEN not in r.text


def test_the_rater_token_is_never_logged(client, rater_roster, capsys):
    """/start prints a participant key when it fails to pipe, because it
    identifies nobody and recovers the run. A rater token is a credential and a
    server log is the wrong place for one — so only the failure mode is logged."""
    client.get("/rate/start", params={"token": VALID_TOKEN}, follow_redirects=False)
    client.get("/rate/start", params={"token": UNKNOWN_TOKEN}, follow_redirects=False)
    out = capsys.readouterr().out
    assert VALID_TOKEN not in out
    assert UNKNOWN_TOKEN not in out


def test_the_rater_link_carries_the_response_id_through(client, rater_roster):
    r = client.get("/rate/start",
                   params={"token": VALID_TOKEN, "qid": "R_0123456789abcde"},
                   follow_redirects=False)
    assert r.headers["location"].endswith("&qid=R_0123456789abcde")


def test_a_response_id_cannot_forge_a_second_query_parameter(client, rater_roster):
    """The value goes into a URL and into a form, so it is checked before it
    stops being data."""
    r = client.get("/rate/start",
                   params={"token": VALID_TOKEN, "qid": "R_1&token=rt_" + "c" * 32},
                   follow_redirects=False)
    assert r.headers["location"] == f"/rate?token={VALID_TOKEN}"


def test_the_console_itself_still_refuses_an_untokened_visit(client, rater_roster):
    """The entrance is a front door, not a way around the lock."""
    assert client.get("/rate").status_code == 401
    assert client.get("/rate", params={"token": UNKNOWN_TOKEN}).status_code == 401
